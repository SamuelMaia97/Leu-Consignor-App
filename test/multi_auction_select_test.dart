  ...
  void main() {
    const auctions = [
      AuctionOption(auctionId: 1, auctionNumber: 1, auctionType: 1, displayName: 'Auction 1'),
      AuctionOption(auctionId: 2, auctionNumber: 2, auctionType: 1, displayName: 'Auction 2'),
    ];

    testWidgets('tapping the field opens the bottom sheet', (tester) async {
      await tester.pumpWidget(MaterialApp(home: Scaffold(body: MultiAuctionSelectField(label: 'Auctions', items: auctions, selected: const [], onChanged: (_) {}))));
    ...
    testWidgets('selecting two auctions and tapping Done calls onChanged with both', (tester) async {
      var selected = <AuctionOption>[];
      await tester.pumpWidget(MaterialApp(home: Scaffold(body: MultiAuctionSelectField(label: 'Auctions', items: auctions, selected: const [], onChanged: (value) => selected = value))));
      ...
      expect(selected.map((e) => e.auctionId), [1, 2]);
    });
    ...
    testWidgets('tapping chip delete removes that auction', (tester) async {
      ...
      await tester.pumpWidget(MaterialApp(home: StatefulBuilder(builder: (context, setState) {
        return Scaffold(body: MultiAuctionSelectField(label: 'Auctions', items: auctions, selected: selected, onChanged: (value) => setState(() => selected = value)));
      })));

      await tester.tap(find.byIcon(Icons.close_rounded).first);
      await tester.pumpAndSettle();

      expect(selected.map((e) => e.auctionId), [2]);
    });
