...
class _ConsignorEditorScreenState extends State<ConsignorEditorScreen> {
  ...
  Future<void> _syncConsignor() async {
    if (_isNew || !_model.shouldUploadDuringWorkspaceSync) return;
  ...
  Future<bool> _saveAsPendingSync() async {
    if (_saveBlockedByActiveSync()) return false;
  ...
  bool _saveBlockedByActiveSync() {
    final state = context.read<AppState>();
    final blocked =
        state.syncingNow || (!_isNew && state.isSyncingConsignor(_model.id));
    if (blocked && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please wait for the current sync to finish.'),
        ),
      );
    }
    return blocked;
  ...
  List<Widget> _buildBottomActions({
    ...
    final actions = <Widget>[
      OutlinedButton.icon(
        onPressed: syncing ? null : _addToDraftAndStay,
        icon: const Icon(Icons.edit_note_outlined),
        label: const Text('Save as draft'),
      ),
      ElevatedButton.icon(
        onPressed: syncing ? null : _saveAndContinue,
  ...
  Widget build(BuildContext context) {
    ...
    final syncing =
        state.syncingNow || (!_isNew && state.isSyncingConsignor(_model.id));
    final showSyncButton = !_isNew && _model.shouldUploadDuringWorkspaceSync;
          ...
          builder: (field) {
            ...
            return InkWell(
              ...
              child: InputDecorator(
                ...
                child: Text(
                  date == null ? 'Select date' : formatDayMonthYear(date),
                  style: date == null
